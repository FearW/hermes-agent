...
bucket = modal.CloudBucketMount(
    ...
    secret: REDACTED"aws-credentials")
)
...
async def verify_token(authorization: REDACTED = Header(None)):
...
async def predict(data: dict, token: REDACTED = Depends(verify_token)):
