...
class ToolEntry:
    ...
    __slots__ = (
        ...
        "max_result_size_chars", "read_only",
    )
    ...
    def __init__(self, name, toolset, schema, handler, check_fn,
        ...
        # True → safe to call during Plan mode (no side effects on user state,
        # external systems, filesystem writes, or message sends).
...
class ToolRegistry:
    ...
    def mark_read_only(self, names) -> int:
        """Flag the named tools as ``read_only=True`` in bulk.

        Used by :mod:`tools.plan_mode` to centrally classify which tools
        a plan-mode advisor may call without side effects.  Returns the
        number of names that matched a registered tool.
        """
        hit = 0
        for n in names:
    ...
    def read_only_names(self) -> Set[str]:
        """Return the set of tool names currently flagged ``read_only``."""
        return {n for n, e in self._tools.items() if e.read_only}
