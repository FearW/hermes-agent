...
class TestCLIQuickCommands:
    ...
    def test_dangerous_exec_command_is_blocked(self):
        cli = self._make_cli({"wipe": {"type": "exec", "command": "rm -rf /tmp/demo"}})
...
class TestGatewayQuickCommands:
    ...
    @pytest.mark.asyncio
    async def test_dangerous_exec_command_is_blocked(self):
        from gateway.run import GatewayRunner

        runner = GatewayRunner.__new__(GatewayRunner)
        runner.config = {"quick_commands": {"wipe": {"type": "exec", "command": "rm -rf /tmp/demo"}}}
        runner._running_agents = {}
        runner._pending_messages = {}
        runner._is_user_authorized = MagicMock(return_value=True)

        event = self._make_event("wipe")
        with patch("asyncio.create_subprocess_shell", new_callable=AsyncMock) as shell_mock:
            result = await runner._handle_message(event)
