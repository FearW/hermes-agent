...
3. append_to_transcript(skip_db=True) suppresses duplicate transcript writes
...
class TestAppendToTranscriptSkipDb:
    """Verify skip_db=True semantics across SQLite and JSONL fallback."""
    ...
    def test_skip_db_writes_jsonl_when_db_unavailable(self, store, tmp_path):
        """With no SQLite store, JSONL remains the transcript fallback."""
    ...
    def test_default_writes_sqlite_only(self, tmp_path):
        """Without skip_db, message is persisted in SQLite only."""
        from gateway.config import GatewayConfig
        from gateway.session import SessionStore
        from hermes_state import SessionDB
