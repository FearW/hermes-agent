...
def test_try_refresh_codex_client_credentials_rebuilds_client(monkeypatch):
    ...
    def _fake_create_openai_client(kwargs, *, reason, shared):
    ...
    assert rebuilt["reason"] == "codex_credential_refresh"
...
def test_preflight_codex_input_strips_null_content_from_function_call_items(
    monkeypatch,
):
    agent = _build_agent(monkeypatch)
    ...
    function_call = next(
        item for item in normalized if item.get("type") == "function_call"
    )
    function_output = next(
        item for item in normalized if item.get("type") == "function_call_output"
    )
    assert "content" not in function_call
    assert "content" not in function_output
