...
class TestStreamingFallback:
    ...
    Since the streaming refactor (PR #3566), _interruptible_streaming_api_call
    no longer falls back to _interruptible_api_call inline.  Instead, it
    propagates errors to the main retry loop which handles recovery
    (credential rotation, CPA model fallback, backoff, and streaming disable).
    Tests verify that errors are correctly propagated.
    """

    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_stream_error_propagates(self, mock_close, mock_create):
        """'not supported' error propagates to caller (main retry loop handles it)."""
        from run_agent import AIAgent
    ...
    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_any_stream_error_propagates(self, mock_close, mock_create):
        """ANY streaming error propagates — the main retry loop decides fallback."""
        from run_agent import AIAgent
    ...
    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_stream_unsupported_sets_disable_flag(self, mock_close, mock_create):
        """'not supported' error sets _disable_streaming flag for next attempt."""
        from run_agent import AIAgent
    ...
    def test_exhausted_transient_stream_error_propagates(self, mock_close, mock_create):
        """Transient stream errors retry, then propagate after retries exhausted."""
        from run_agent import AIAgent
        import httpx
    ...
    def test_sse_connection_lost_retried_as_transient(self, mock_close, mock_create):
        ...
        with pytest.raises(OAIAPIError, match="Network connection lost"):
            agent._interruptible_streaming_api_call({})
    ...
    def test_sse_non_connection_error_propagates_immediately(self, mock_close, mock_create):
        ...
        with pytest.raises(OAIAPIError, match="Invalid model configuration"):
            agent._interruptible_streaming_api_call({})
