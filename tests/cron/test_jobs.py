...
class TestParseSchedule:
    ...
    def test_naive_timestamp_uses_configured_timezone(self, monkeypatch):
        monkeypatch.setenv("HERMES_TIMEZONE", "Asia/Shanghai")
        reset_cache()
        try:
            result = parse_schedule("2030-01-15 14:00")
            run_at = datetime.fromisoformat(result["run_at"])
            assert run_at.utcoffset() == timedelta(hours=8)
            assert run_at.hour == 14
            assert run_at.isoformat() == "2030-01-15T14:00:00+08:00"
        finally:
            reset_cache()

    def test_aware_timestamp_preserves_explicit_offset(self, monkeypatch):
        monkeypatch.setenv("HERMES_TIMEZONE", "Asia/Shanghai")
        result = parse_schedule("2030-01-15T14:00:00+02:00")
        run_at = datetime.fromisoformat(result["run_at"])
        assert run_at.isoformat() == "2030-01-15T14:00:00+02:00"
