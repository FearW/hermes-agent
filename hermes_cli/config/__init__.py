...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 9:
        try:
            ...
            logger.debug("Failed to clear legacy ANTHROPIC_TOKEN: REDACTED", exc)
    ...
    if current_ver < 13:
        for dead_var in ("LLM_MODEL", "OPENAI_MODEL"):
            try:
                ...
                logger.debug("Failed to clear legacy %s: %s", dead_var, exc)
...
# Core config loader -- extracted to config/loader.py
from hermes_cli.config.loader import (
    _deep_merge,
    _expand_env_vars,
...
def show_config():
    ...
    try:
        ...
        logger.debug("Failed to display skill settings: %s", exc)
...
# Compatibility shim for gateway modules restored from official main.
...
def _normalize_custom_provider_entry(
    ...
    """Normalize one legacy/new custom provider entry for compatibility views."""
    if not isinstance(entry, dict):
        return None
...
def providers_dict_to_custom_providers(providers: Any) -> List[Dict[str, Any]]:
    """Return list-shaped custom providers from the newer providers mapping."""
...
def get_compatible_custom_providers(
    ...
    """Return a deduplicated list view across legacy and providers config."""
    if config is None:
        config = load_config()
    ...
    def append_if_new(entry: Optional[Dict[str, Any]]) -> None:
        if entry is None:
            return
    ...
    if isinstance(custom_providers, list):
        for entry in custom_providers:
            append_if_new(_normalize_custom_provider_entry(entry))
