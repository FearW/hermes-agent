...
def get_pricing(
    ...
    """Return legacy per-token pricing values for insights callers."""
    entry = get_pricing_entry(model_name, provider=provider, base_url=base_url, api_key=api_key)
    if not entry:
        return dict(DEFAULT_PRICING)
    input_cost = entry.input_cost_per_million or _ZERO
    output_cost = entry.output_cost_per_million or _ZERO
    return {
        "input": float(input_cost / _ONE_MILLION),
        "output": float(output_cost / _ONE_MILLION),
    }
